...
public class PlatformerController : MonoBehaviour 
{
    ...
    private Vector3 playerEyes;
    private Vector3 playerChest;
    ...
    public class PlatformerControllerMovement 
    {
        ...
        // Is the user against a wall, but not grounded?
        [System.NonSerialized]
    ...
    public class PlatformerControllerJumping 
    {
        ...
        // The speed of a wall jump
        [System.NonSerialized]
        public float wallJumpSpeed = 25.0f;
    ...
    // Sound and particle effects for platforms
    public CollisionParticleEffect groundParticle;
    public CollisionSoundEffect groundSound;
    ...
    {	
        ...
        else
        {
		    ...
		    if (movement.isMoving)
            {
                movement.inAirVelocity += new Vector3 (h, 0, 0) * Time.deltaTime * movement.inAirControlAcceleration;
            }
	    }
    }
    ...
    void FixedUpdate() 
    {
        ...
        playerEyes = transform.position + Vector3.up * 1.5f;
        playerChest = transform.position + Vector3.up * 1.2f;
    }
    ...
    void ApplyJumping() 
    {
        ...
        else
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(playerChest, fwd, out hit, 0.6f))
            {
                // Don't allow player to walljump off a rigidbody
                if (hit.rigidbody || controller.isGrounded)
                {
                ...
                if (jump.enabled && Time.time < jump.lastButtonTime + jump.timeout)
                {
                    ...
                    movement.speed = jump.wallJumpSpeed;
                    movement.direction = transform.TransformDirection(-transform.forward);

                    SendMessage("DidJump", SendMessageOptions.DontRequireReceiver);

                    canControl = false;
                    movement.onWall = false;
                    Invoke("AllowControl", .1f);
                }
            }
            else
            {
    ...
    void ApplyGravity() 
    {
        ...
        else if (movement.onWall)
        {
            if (movement.verticalSpeed > 0)
            {
                movement.verticalSpeed = 0;
            }
            movement.verticalSpeed -= movement.wallGravity * Time.deltaTime;
        }
        else
            movement.verticalSpeed -= movement.gravity * Time.deltaTime;
    ...
    void Update () 
    {
        ...
        //Debug.Log("PREVIOUS POSITION: " + lastPosition);
    ...
    {
        //Debug.Log("Y: " + hit.moveDirection.y);
        //Debug.Log("X: " + hit.moveDirection.x);
        //Debug.Log("NORMAL Y: " + hit.normal.y);
        //Debug.Log("NORMAL X: " + hit.normal.x);

        // Touching bottom of platform
	    if (hit.moveDirection.y > 0.01 || hit.rigidbody)
		    return;
        ...
        // Grab particle and sound effects from the ground the character is on
        CollisionParticleEffect collisionParticleEffect = hit.collider.GetComponent<CollisionParticleEffect>();
        if (collisionParticleEffect)
        {
            groundParticle = collisionParticleEffect;
        }

        CollisionSoundEffect collisionSoundEffect = hit.collider.GetComponent<CollisionSoundEffect>();
        if (collisionSoundEffect)
        {
            groundSound = collisionSoundEffect;
        }
    }
